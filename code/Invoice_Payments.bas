Attribute VB_Name = "Invoice_Payments"
' qbooks.bas
'
...
Public lInvoiceNextPlace As Long
...
On Error GoTo errhandler
    ...
    qbXMLRP.OpenConnection "QBRcvPymt", "QuickBooks Sample Receive Payment"
...
On Error GoTo ErrHandlerBeginSession
    ...
    ticket = qbXMLRP.BeginSession(companyFile, qbFileOpenMultiUser)
...
Private Function genListsRequest(customer As String) _
    ...
    'Set invQueryNode = doc.createElement("ReceivePaymentQueryRq")
    ...
    Set includeTxnNode = doc.createElement("IncludeLineItems")
    includeTxnNode.Text = "true"
    invQueryNode.appendChild includeTxnNode
...
Private Sub processListsResponse(outXML As String, _
    ...
    'check for 15 days back for any payment info
    Dim dteTxnDate As Date
    Dim dtesDateToCheckFor As Date
    ...
    Dim i As Integer, j As Integer, k As Integer   ' Will be a for-loop index
    ...
    Dim txnIDNodeList As IXMLDOMNodeList
    Dim txnIDNode As IXMLDOMNode
    Dim txnDateNode As IXMLDOMNode
    Dim AmountNode As IXMLDOMNode
    Dim LineItemNodeList As IXMLDOMNodeList
    Dim LineItemNode As IXMLDOMNode
    Dim LineItemRefNode As IXMLDOMNode
    Dim LineItemRefNodeList As IXMLDOMNodeList
    Dim sReceivedPayementsArray() As String
    ...
    If 0 = statusCode1 Or 530 = statusCode1 Then
        ...
        For i = 0 To numInvs - 1
            ...
            Set txnIDNodeList = invoiceNode.selectNodes("LinkedTxn")
            ReDim sReceivedPayementsArray(txnIDNodeList.length, 2)
            
            For j = 0 To txnIDNodeList.length - 1
                Set txnIDNode = txnIDNodeList.Item(j)
                Set txnDateNode = txnIDNode.selectSingleNode("TxnDate")
                Set AmountNode = txnIDNode.selectSingleNode("Amount")
                'Debug.Print txnDateNode.Text & " : " & AmountNode.Text
                sReceivedPayementsArray(j, 0) = txnDateNode.Text
                sReceivedPayementsArray(j, 1) = AmountNode.Text
            ...
            Set LineItemNodeList = invoiceNode.selectNodes("InvoiceLineRet")
            
            If invRefNum = "4734" Then
                invRefNum = invRefNum
            End If
            
            If LineItemNodeList.length > 0 And lInvoiceNumber = invRefNum Then
                For j = 0 To LineItemNodeList.length - 1
                    Set LineItemNode = LineItemNodeList.Item(j)
                    'Debug.Print LineItemNode.selectSingleNode("Quantity").Text
                    InvoicesDetails(InvoiceNum, j, 0) = invRefNum
                    InvoicesDetails(InvoiceNum, j, 1) = LineItemNode.selectSingleNode("Quantity").Text
                    InvoicesDetails(InvoiceNum, j, 2) = LineItemNode.selectSingleNode("Desc").Text
                    InvoicesDetails(InvoiceNum, j, 3) = LineItemNode.selectSingleNode("Rate").Text
                    InvoicesDetails(InvoiceNum, j, 4) = LineItemNode.selectSingleNode("Amount").Text
                    InvoicesDetails(InvoiceNum, j, 6) = LineItemNodeList.length
                    'Debug.Print "-->(" & InvoiceNum & ") invoice:" & invRefNum & ", amount:" & InvoicesDetails(InvoiceNum, j, 4)
                    
                    Set LineItemRefNodeList = LineItemNode.selectNodes("ItemRef")
                    
                    If LineItemRefNodeList.length > 0 Then
                        For k = 0 To LineItemRefNodeList.length - 1
                            Set LineItemRefNode = LineItemRefNodeList.Item(k)
                            InvoicesDetails(InvoiceNum, j, 5) = LineItemRefNode.selectSingleNode("FullName").Text
                        Next
                    End If
                Next
                Invoicedone = InvoiceNum
            End If
            ...
            Set testNodeList = invoiceNode.selectNodes("TxnDate")
            If 1 = testNodeList.length Then
                Set invRefElement = invoiceNode.selectSingleNode("TxnDate")
                sTxnDate = invRefElement.Text
            Else
                sTxnDate = ""
            End If
            ...
            If lInvoiceNumber = invRefNum Then
            
                dteTxnDate = sTxnDate
                dtesDateToCheckFor = DateAdd("d", Now, -15)
                
                'If dteTxnDate >= dtesDateToCheckFor Then
                    frmInvoiceQry.AddInvoiceToList txnId, appliedAmt, balanceRemaining, _
                        suggDiscount, invRefNum, i, sTxnDate, sReceivedPayementsArray
                'End If
            End If
        Next
        ...
        If 0 = statusCode2 Or 530 = statusCode2 Then
        ...
        Else
            ...
            Else
                ...
                MsgBox "1Error from QuickBooks: " & statusMsg2
    ...
    Else
        ...
        Else
            ...
            MsgBox "2Error from QuickBooks: " & statusMsg1
...
Public Function genRecPaymentRequest(customer As String, _
    ...
    Dim depToAccountNode As IXMLDOMElement
    Set depToAccountNode = doc.createElement("DepositToAccountRef")
    recPayNode.appendChild depToAccountNode
    
    Dim depAccountName As IXMLDOMElement
    Set depAccountName = doc.createElement("FullName")
    depAccountName.Text = "Undeposited Funds"
    depToAccountNode.appendChild depAccountName
...
Public Function processRecPaymentResponse(strResponseXML As String) As Boolean
    ...
    If 0 = statusCode Or 530 = statusCode Then
...
'
Public Function currToString(curr As String) As String
...
Function qbXMLLatestVersion(rp As RequestProcessor, ticket As String) As String
    ...
    Dim QueryResponse As New DOMDocument40
    
    qbXMLLatestVersion = "1.1"
    'Parse the response XML
    QueryResponse.async = False
    If (QueryResponse.loadXML(strXMLResponse)) Then

        Dim supportedVersions As IXMLDOMNodeList
        Set supportedVersions = QueryResponse.getElementsByTagName("SupportedQBXMLVersions")
